...
with st.sidebar:
    ...
    if search_q:
        ...
        if hits:
            # 검색 결과 라벨도 익명화(실명 등록사는 'A사'로). 선택 매핑은 인덱스로 유지.
            opts = [f"{_alias(h['corp_name'])}" + ("" if _alias(h['corp_name']) != h['corp_name']
                    else f" ({h['corp_code']})") for h in hits]
    ...
    # 시연 익명화: 등록된 실명은 화면 표시용으로 익명 치환 (DART 조회는 corp_code 그대로)
    from esgenie.demo_aliases import display_name as _demo_display_name
    _orig_name = corp_name
    corp_name = _demo_display_name(corp_name)
    if corp_name != _orig_name:
        st.caption("🔒 시연 익명화 적용 — 실명 대신 익명으로 표시 (DART 데이터는 실제 사용)")
...
st.caption("한전 전기요금 고지서 · 도시가스 영수증 · 폐기물 대장 · 안전보건 회의록 · 사내 규정집 · OEM SAQ(자가진단 설문) PDF")
...
if "upload_roles" not in st.session_state:
...
if uploads:
    ...
    for uf in uploads:
        ...
        role = "supplier_claim" if is_saq_upload(str(p), file_name=uf.name) else "evidence"
        ...
        rows.append({"파일명": uf.name, "채널": dec.channel.value,
                     "문서 유형": dec.doc_type, "연동": "SAQ 자가주장" if role == "supplier_claim" else "증빙",
...
def _run_pipeline():
    ...
    evidence_files = {
        name: path for name, path in upload_paths.items()
        if roles.get(name) != "supplier_claim"
    }
    saq_paths = [
        path for name, path in upload_paths.items()
        if roles.get(name) == "supplier_claim"
    ]
    ...
    result.supplier_claim_files = [Path(p).name for p in saq_paths]
    return result
