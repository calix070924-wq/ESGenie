...
| 신호 | 이름 | 내용 |
|------|------|------|
| A | 민감 항목 누락 | 배출량(E-3)·폐기물(E-6)·오염(E-7)·법규위반(E-8/S-9/G-6)·산재(S-4-2) 등 "숨기고 싶은" 항목이 프로파일 대상인데 누락. **누락된 민감 항목 가중의 합 ÷ 프로파일 내 민감 항목 총가중**(비율식) |
| B | 고아 비율 (가장 강한 신호) | 유리한 *비율*만 공시하고 그 분모·총량은 침묵. 예: 폐기물 재활용률(E-6-2)은 자랑, 폐기물 총량(E-6-1)은 누락. `min(1, 고아건수 ÷ 가능한 비율-맥락 페어 수 × 2.0)` |

점수 = `min(1, 0.45 × 신호A + 0.55 × 신호B)` — 고아 비율(강신호)에 가중을 둔다
(`layer3_disclosure.py:168·192·202` 기준).

```bash
python -m esgenie.pipeline --ticker 005930 --areas E        # D6 의심도·고아비율 CLI 출력
python -m esgenie.benchmark_disclosure                       # 문서단위 벤치 15케이스
```

> 문서단위 벤치(`data/benchmark_v2/disclosure_bench.json`, 15케이스 = 설계 12 + 실보고서 3):
> **레벨 정확도 80.0% · 이진 86.7%.** 실보고서 3건(LG전자 2025-2026 / KB금융 2023 /
> 현대모비스 2025)은 각 사 ESG Data Appendix 실측 입력이며 전건 정답.
>
> **알려진 미탐 3건**(D6-08 / D6-09 / D6-10)은 모두 **고아 비율 없이 민감 항목만 대량 누락**한
> 시나리오다. 신호A가 프로파일 내 민감 항목 총가중을 분모로 쓰는 비율식이라 대량 누락이 큰
> 분모에 희석된다(D6-08 = 위반·배출·산재 6건 은폐인데 score 0.231 → low). 신호A를 절대
> 누적식으로 바꾸면 해소되지만 벤치 전체 재캘리브레이션이 따라오므로 조정하지 않고
> 남겨둔다 — 12케이스는 직접 설계·튜닝한 셋이라 그 위의 100%는 일반화 근거가 아니었고,
> 실보고서 3건을 넣어 80%로 떨어진 지금 수치가 더 정직한 기준선이다.
...
> **test(held-out, n=270, 2026-07-12 실키 측정)**: F1 **0.522**(95% CI 0.404–0.629) /
> Precision 0.938(0.833–1.000) / Recall 0.361(0.259–0.468). 같은 임계값에서 dev(튜닝셋 n=50)는
> F1 1.000이다.
> **동일도메인(실보고서) 특이도 0.989**(clean 187문장 중 185건 올바른 미플래그, CI 0.973–1.000) —
> 12개사 실보고서에서 수집한 문장이며, "진짜 보고서 언어를 그린워싱으로 과검출하지 않는다"가
> 이 시스템의 실제 강점이다. 반면 **교차도메인(광고) recall은 0.393**(56건 중 22건)으로,
> 특히 영어 마케팅 문구(ASA 출처 신규 16건)는 전건 미탐이다.
>
> dev 1.000 → test 0.522의 격차 자체가 held-out 분리의 산출물이다. 분리하지 않았다면 dev 1.0을
> 성능으로 오인했을 것이고, 미탐 16건이라는 일반화 격차도 드러나지 않았다. 임계값을 사후
> 조정해 test 점수를 올리지 않는다(`docs/held_out_eval_methodology.md` §5).
