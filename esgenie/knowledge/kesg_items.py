...
_SEARCH_TERMS: dict[str, tuple[str, ...]] = {
    ...
    "S-3-1": ("여성 구성원", "여성 비율", "여성 인력", "여성 직원 비율"),
    ...
    "G-2-1": ("이사회 출석률", "전체 이사 출석", "이사회 참석률", "이사 출석률",
              "이사 평균 참석률"),
...
def kesg_name(code: str) -> str:
    """K-ESG 코드 → 항목명. 모르는 코드는 코드 자체를 돌려준다.

    표시·프롬프트용 항목명의 단일 출처. 이전에는 detector_5axis와 audit_trace가
    각자 하드코딩 표를 들고 있었고, 양쪽 모두 실제 항목명과 어긋나 있었다
    (S-4-1을 '정보보호 정책'으로 적어 안전보건 체크리스트가 정보보호 항목명으로
    LLM에 넘어갔다 — 2026-07-28 정정).
    """
    item = by_code(code)
...
def resolve_kesg_code(
    ...
    sub = [(alias, code, n.find(alias)) for alias, code in _ALIAS_UNIQUE.items()
           if len(alias) >= 2 and alias in n]
    if sub:
        # leftmost-longest: 더 구체적인 긴 별칭 우선, 동길이면 라벨에서 먼저 나온 별칭 우선.
        # "Scope 3 … Scope 1이나 2에 포함되지 않는 …"처럼 설명문에 다른 Scope가 뒤따라도
        # 앞의 지표명이 이긴다. 길이·위치까지 같은데 코드가 다를 때만 모호로 기권한다.
        sub.sort(key=lambda x: (-len(x[0]), x[2]))
        top_len = len(sub[0][0])
        top_pos = sub[0][2]
        winners = {
            code for alias, code, pos in sub
            if len(alias) == top_len and pos == top_pos
        }
        if len(winners) == 1:
