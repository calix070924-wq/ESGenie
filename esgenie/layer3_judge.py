...
- 다만 다음 셋 중 하나면 다짐이라도 → confirmed(위험):
  ...
  ③ **친환경·탄소중립·ESG·지속가능 등을 상품·서비스의 수식어로 붙여 소비자에게 구매·참여를
     권유하는 광고·홍보 문구**("친환경 항공권으로 ~하세요", "ESG 친환경 펀드로 ~투자하세요")는
     실질 배출저감·심사기준 등 구체 근거가 문장에 없으면 → confirmed(모호한 친환경 라벨).
- 단, ②·③의 환경 키워드라도 **기업이 자신의 활동을 '추진/노력/목표'로 서술하는 진행형 다짐**
  ("친환경 경영을 추진하고 있습니다", "탄소중립 달성을 위해 노력")은 성취 단정이 아니므로
  → false_positive(해제). ③ 소비자를 향한 권유문(~하세요)과 기업의 진행형 다짐(~을 추진/노력/
  목표)을 구분하라: 권유문 = 광고 → confirmed; 기업 다짐 = 포부 → false_positive.
...
예시5 (상품 친환경 라벨 권유 → 확정): "친환경 항공권으로 친환경 여행에 동참하세요."
  · 실질 배출저감 근거(항공기 효율·SAF 비율 등) 없음 — 소비자 권유 광고
  · D2_modifier=confirmed (근거 없는 상품 친환경 라벨 권유 — ③ 적용)
예시6 (ESG 펀드 라벨 권유 → 확정): "ESG 친환경 투자 펀드로 지속가능한 미래에 투자하세요."
  · 운용·심사기준 등 구체 근거 없음 — 소비자 권유 광고
  · D2_modifier=confirmed (근거 없는 상품 ESG 라벨 권유 — ③ 적용)"""
