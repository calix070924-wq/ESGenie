...
_KESG_PATTERNS: list[tuple[str, str, str, float]] = [
    ...
    # ── S-4-2 산업재해율 ───────────────────────────────────────────────────
    # (2026-07-28: S-3-1은 '여성 구성원 비율'이라 재해율이 여성비율 노드로
    #  들어가고 있었다. DART 직원현황 API가 채우는 S-3-1과 값이 충돌.)
    ("S-4-2", "%",      r"(?:산재율|재해율|사고율)[^\d]{0,30}([\d,\.]+)\s*%", 1.0),
    ("S-4-2", "건",     r"(?:산재|재해|사고\s*건수)[^\d]{0,30}([\d,\.]+)\s*건", 1.0),
...
_KESG_SANITY: dict[tuple[str, str], tuple[float, float]] = {
    ...
    ("E-6-1", "ton"):    (1.0, 1e9),
    ("S-4-2", "%"):      (0.0, 10.0),        # 재해율 — 10%를 넘으면 다른 지표를 집은 것
    ("S-3-1", "%"):      (0.0, 100.0),       # 여성 구성원 비율 — 통상 10~40%
    ("G-3-1", "일"):     (0.0, 3650.0),
}
